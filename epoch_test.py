...
if __name__ == '__main__':
    ...
    prob_test = {"N Epochs": [], "AUC": []}

    for num_epochs in nep_to_test:
        for repeat in range(n_reps):
            print(f"Beginning repeat {repeat} for {num_epochs} epochs")
            ...
            prob_test["N Epochs"].append(num_epochs)
            prob_test["AUC"].append(prob_auc)

            print(f"Test scores for {num_epochs} epochs")
            print(f"Prob AUC: {prob_auc}")
            print()

    fig = pl.figure()
    sns.boxplot(data=prob_test, x="N Epochs", y="AUC")
    sns.stripplot(data=prob_test, x="N Epochs", y="AUC", color='k')
    sns.despine()
